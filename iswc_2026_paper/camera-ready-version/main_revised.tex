...
Consequently, availability, latency, interoperability, timeouts, and usage limits can vary substantially in practice~\cite{Vandenbussche2017SPARQLES,kartographi2022}; such operational constraints are therefore not exceptional failures, but recurring factors that shape federated query execution in these real-world environments.
...
Beyond simply measuring query success and failure, we capture temporal variation in execution behavior through a longitudinal study spanning 18 months, during which the same set of queries was executed across over 20 public SPARQL endpoints. 
...
\input{query_content/example_query}
...
In real-world conditions, public endpoints can vary substantially in availability, interoperability, and performance~\cite{Vandenbussche2017SPARQLES,kartographi2022}, and these factors can directly affect federated query execution, sometimes causing partial or complete failure.
Work has been done to mitigate these challenges, an example is SPARQL continuation queries \cite{passage2025}, but such solutions have not yet been adopted at scale.
...
This study is an 18-month longitudinal evaluation of SPARQL query federation over public endpoints, focusing on the practical executability of real federated queries rather than controlled benchmark performance. 
...
\input{query_content/query_complexity}

Of the original 67 queries, 7 encountered errors early in experimentation and were excluded from \textsc{no-service} experiments.
...
\input{methods/comunica_configs_table}
...
The testing campaign comprised 25 experiments conducted between March 2025 and June 2026. 
...
% General findings
An interactive all-query outcome heatmap, showing all experimental query executions chronologically, is shown in Figure~\ref{fig:allQueries} and is available in the \textit{Overview Figures} section of the \href{https://ecrum19.github.io/fed-survey-results/}{results webpage}.
...
In contrast, \textsc{no-service} experiments had only a \(2.3\%\) \emph{execution success rate}, and \(96.3\%\) failed with explicit errors.
...
Winter and Spring 2026 remained low-success, high-error phases, with \emph{execution success rates} of \(2.1\%\) and \(1.5\%\), and error rates of \(96.7\%\) and \(98.05\%\), respectively.
...
The most frequent were \textsc{client-side HTTP} errors (n=850; 4xx endpoint rejections), \textsc{server-side HTTP} errors (n=242; 5xx failures), \textsc{other} errors (n=102; e.g., HTTP 200 responses containing HTML error pages), \textsc{transport-level} failures (n=82; incomplete responses such as \texttt{fetch failed}), and \textsc{timeout-related} errors (n=63). 
...
We examined whether successful \textsc{no-service} executions differed from failures in HTTP request burden, restricting the analysis to runs with valid request counts ($n=1260$). 
...
Under a broad success definition, where success means any execution without an explicit error, successful runs issued more requests than failures on average (46{,}366 vs. 4{,}161) and by median (22 vs. 5). 
The association was positive but modest ($r=0.223$) and supported by permutation tests ($p=3.33\times10^{-5}$ for correlation; $p=3.33\times10^{-5}$ for mean difference) and by a Mann--Whitney U test ($p=1.33\times10^{-15}$, one-sided).
Using a stricter definition of success as a non-empty result set, successful executions again showed higher request burden, both in mean (68{,}085 vs. 4{,}267) and median (580 vs. 5). 
The association remained moderate ($r=0.266$) and was again supported by permutation tests ($p=3.33\times10^{-5}$ for correlation; $p=3.33\times10^{-5}$ for mean difference) and by a Mann--Whitney U test ($p=2.04\times10^{-12}$, one-sided).
...
However, the large mean--median gaps show that request burden remained highly uneven and dominated by outliers, making HTTP request count alone an incomplete predictor of success. 
...
Across the \textsc{no-service} experiments, all client-side automated federation algorithm families failed in most executions, with \emph{execution success rates} between only \(1.67\%\) and \(2.50\%\).
The corresponding counts were \(12/480\) for \textsc{NOMETA-ASK}, \(5/300\) for \textsc{NOMETA-COUNT}, \(6/240\) for \textsc{VOID-TRIPLE}, and \(6/240\) for \textsc{VOID-BLOCK}.
These results are visualized in the \textsc{no-service} \textit{algorithm efficacy comparison} graph on the \href{https://ecrum19.github.io/fed-survey-results/}{results webpage}.

Thus, all algorithms succeeded in a small minority of cases, but none were broadly robust for the tested workload. 
Differences between methods were modest: \textsc{VOID-TRIPLE} had the highest explicit-error-free rate, \textsc{VOID-BLOCK} the highest \emph{execution success rate}, and \textsc{NOMETA-COUNT} was consistently weakest. 
...
The \textsc{-NRL} experiments show that client-side rate-limit mitigation did not seem to impact \emph{execution success rates}. 
...
Crucially, this delicate balance between data requester and data provider is not restricted to the public SPARQL endpoints tested in this study, but highlights significant emerging challenges to modern public data access in general.
...
Elias Crum was supported by the Research Foundation – Flanders (FWO) (SB Fellowship 1S27825N) and (Long Research Stay V416635N).
Ruben Taelman is an FWO postdoctoral fellow (1202124N).
Tarcisio Mendes de Farias and Ana Sima were supported by the Swiss National Science Foundation through the MetaboLinkAI project (10.002.786), European Union’s Horizon Europe research and innovation programme (grant No. 101131818 - project MICROBES-4-CLIMATE), and the Swiss Federal Government State Secretariat for Education, Research and Innovation (SERI).
Jonni Hanski and Bryan-Elliot Tam were supported by SolidLab Vlaanderen (Flemish Government, EWI and RRF project VV023/10).
Jerven Bolleman as member of the Swiss-Prot group and UniProt was supported by the NHGRI, Office of Director [OD/DPCPSI/ODSS]; NIAID, NIA, NIGMS, NIDDK, NEI, NCI, and NHLBI of the National Institutes of Health (NIH) [U24HG007822]; additionally supported by the Swiss Federal Government through SERI.
\end{credits}
